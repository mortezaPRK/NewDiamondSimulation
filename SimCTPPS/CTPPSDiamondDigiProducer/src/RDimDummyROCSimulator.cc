...
RDimDummyROCSimulator::RDimDummyROCSimulator(const edm::ParameterSet &params, uint32_t det_id)
...
void RDimDummyROCSimulator::ConvertChargeToHits(const std::map<unsigned short, double, std::less<unsigned short> > &signals, 
...
//  the_trig_cont_links_.clear();
        ...
        i!=signals.end(); ++i)
  {
    ...
    unsigned short pixel_no = i->first;   // questo dovrebbe essere il PixelIndex di CTPPSDiamondSimTopology.h (col*160+row)
...
/// set maximum for 8 bits adc
      ...
      output_digi.push_back(CTPPSDiamondDigi(row,col,adc) );//(det_id_, pixel_no)); /// ?????????????????????????????????????? devo metterci row, col e conteggi adc
...
void RDimDummyROCSimulator::SetDeadPixels()
{
  dead_pixels_.clear();
