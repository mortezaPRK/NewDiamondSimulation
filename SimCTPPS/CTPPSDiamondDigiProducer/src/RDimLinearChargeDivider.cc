    ...
    uint32_t det_id) : params_(params), rndEngine(eng) , _det_id(det_id)
{
  ...
  fluctuateCharge_ = params.getParameter<bool>("RDimLandauFluctuations");
...
//  chargedivisionsPerStrip_ = params.getParameter<int>("RDimChargeDivisionsPerStrip");
//  chargedivisionsPerThickness_ = params.getParameter<int>("RDimChargeDivisionsPerThickness");
  chargedivisions_ = params.getParameter<int>("RDimChargeDivisions");
  ...
  deltaCut_ = params.getParameter<double>("RDimDeltaProductionCut");
/*
...
void RDimLinearChargeDivider::FluctuateEloss(int pid, double particleMomentum, 
...
//  else dedx = eloss;
  ...
  double particleMass = RDimHepPDTWrapper::GetMass(pid)*1000; //to have the mass in MeV
  if(particleMass==-1)
